...
export default defineComponent({
  setup() {
    ...
    onMounted(() => {
      if (state.store && state.store.formaPagamento) {
        state.nome = state.store.formaPagamento.data.nome;
        state.parcelas = state.store.formaPagamento.data.parcelas;
      }
    });
    ...
    const save = async () => {
      try {
        ...
        let response = await FormaPagamentoService.update(
          {
            nome: state.nome,
            parcelas: state.parcelas,
          },
          state.store.formaPagamento.data.id,
          router
        );
      } catch (error) {
        console.log(error);
