    ...
    <div class="container-right">
      ...
      <div style="overflow-y: auto; height: 30vh">
        <table id="selectionedProducts" class="table-container">
          ...
          <tbody>
            <tr v-for="(product, index) in state.selectedProducts" :key="index">
              <td>{{ index + 1 }}</td>
              <td>{{ product.nome }}</td>
              <td>{{ product.preco }}</td>
              <td>
            ...
            <tr v-if="state.selectedProducts.length === 0">
              <td colspan="5">
  ...
  setup() {
    ...
    async function getClientes() {
      try {
        ...
        state.clientes = clientes.data.data;
      } catch (error) {
        console.log(error);
      }
    }
    ...
    async function getFormaPagamento() {
      try {
        ...
        state.formaPagamento = formaPagamento.data.data;
      } catch (error) {
        console.log(error);
      }
    }
    ...
    onMounted(async () => {
      await getProdutos();
      await getClientes();
      await getFormaPagamento();

      if (state.store && state.store.venda) {
        state.selectedProducts = state.store.venda.data.produtos;
        state.selectedProducts.forEach((selectedProduct) => {
          ...
          const index = state.produtos.findIndex(
            (product) => product.id === selectedProduct.codigo
          );
          if (index !== -1) {
            state.produtos.splice(index, 1);
          }
        });
        state.total = state.selectedProducts.reduce((acc, product) => {
          return acc + product.preco * product.quantidade;
        }, 0);
      }
    });
        ...
        await schema.validate(
          {
            ...
            produtos: state.selectedProducts,
            formaPagamento: state.formaPagamentoId,
          },
          { abortEarly: false }
        );
        let data = {
          total: state.selectedProducts.reduce((acc, product) => {
            state.total = acc + product.quantidade * product.preco;
            return acc + product.quantidade * product.preco;
          }, 0),
          produtos: state.selectedProducts.map((product) => ({
            codigo: product.id || product.codigo,
            quantidade: product.quantidade,
          })),
          formaPagamentoId: parseInt(state.formaPagamentoId),
          clienteId: parseInt(state.clienteId),
        };
        await VendasServices.update(data, state.store.venda.data.id, router);
      } catch (error) {
    ...
    function handleQuantityChange() {
      state.total = state.selectedProducts.reduce((acc, p) => {
        return acc + p.quantidade * p.preco;
      }, 0);
    }

    function incluirProduto(product) {
      if (!state.selectedProducts.some((p) => p.id === product.id)) {
        const index = state.produtos.findIndex((p) => p.id === product.id);
        state.selectedProducts.splice(index, 0, product);
        state.produtos.splice(index, 1);
        state.total += product.preco * product.quantidade;
      }
    }

    function removeProduto(product) {
